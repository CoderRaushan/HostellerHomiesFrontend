...
function Complaints() {
  ...
  useEffect(() => {
    ...
    const dates = [
      ...
      new Date(Date.now() - 0 * 24 * 60 * 60 * 1000).toLocaleDateString("en-US", {
        day: "numeric", month: "long", year: "numeric"
      }),
    ];
  ...
  return (
    <div className="w-full h-full flex flex-col gap-10 md:gap-7 items-center justify-start pt-36 overflow-auto bg-[#f3e8ff]">
      <h1 className="text-[#4f46e5] font-bold text-5xl">Complaints</h1>
      <div className="flex md:gap-7 flex-wrap justify-center items-center gap-7">
        {graph}
        <div className="bg-white px-10 py-5 rounded-xl shadow-xl w-96 max-h-64 overflow-auto">
          <span className="text-[#4f46e5] font-bold text-xl">New Complaints</span>
          <ul role="list" className="divide-y divide-[#4f46e5] text-black">
            {unsolvedComplaints.length === 0
              ? "No new complaints!"
              : unsolvedComplaints.map((complaint) => (
                  <li
                    className="py-3 sm:py-4 px-5 rounded hover:bg-gray-200 hover:scale-105 transition-all"
                    key={complaint.student}
                  >
                    <div className="flex items-center space-x-4">
