...
const Input = ({ field }) => (
  <div className="mb-2">
    <label htmlFor={field.name} className="block mb-2 text-sm font-medium text-gray-800">
...
function Complaints() {
  ...
  const fetchComplaints = async () => {
    try {
      const res = await axios.post(`${import.meta.env.VITE_MAIN_URI}/api/complaint/student`, {
        student: student._id,
      });
  ...
  const registerComplaint = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_MAIN_URI}/api/complaint/register`, {
        student: student._id,
      ...
      if (res.status === 200 || res.status === 201) {
        setNotification({ message: "Complaint Registered Successfully!", type: "success" });
        setTitle("");
        setDesc("");
        setType("Electric");
        fetchComplaints();
  ...
  return (
    <div className="min-h-screen w-full p-6 mt-10 flex flex-col items-center justify-start bg-white">
      <h1 className="font-bold text-4xl mt-6 mb-8 text-indigo-700">Complaints</h1>
      ...
      <div className="flex gap-8 flex-wrap items-start justify-center max-w-5xl">
        {/* Complaint Form */}
        <form onSubmit={registerComplaint} className="w-full md:w-96 p-6 bg-white rounded-xl shadow-lg flex flex-col gap-5 border border-indigo-100">
          <h2 className="text-xl font-semibold text-indigo-700">Register New Complaint</h2>

          <div>
            <label htmlFor="type" className="block mb-2 text-sm font-medium text-gray-700">Complaint Type</label>
            <select
          ...
          <div>
            <label htmlFor="description" className="block mb-2 text-sm font-medium text-gray-700">Complaint Description</label>
            <textarea
        ...
        <div className="w-full md:w-96 max-h-96 p-5 rounded-xl shadow-lg bg-white border border-indigo-100 overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-indigo-700">Registered Complaints</h2>
          </div>
