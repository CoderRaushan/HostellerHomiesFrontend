...
function Suggestions() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#f3e8ff] px-3 sm:px-5 md:px-8 py-6">
      <div className="max-w-6xl mx-auto pt-12 sm:pt-16 md:pt-20">
        <h1 className="text-[#4f46e5] font-bold text-2xl sm:text-4xl md:text-5xl text-center mb-6 sm:mb-10">
          Share Your Suggestions
        </h1>

        <p className="text-center text-base sm:text-lg font-medium mb-6 text-gray-700">
          You’ve submitted{" "}
          <span className="font-bold text-[#4f46e5]">{count}</span> of 2
          suggestions today.
        </p>
        ...
        <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 justify-center items-start">
          {/* Suggestion Form */}
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 w-full lg:w-1/2 flex flex-col justify-between min-h-[460px] sm:min-h-[500px]">
            <form onSubmit={registerSuggestions} className="space-y-5 sm:space-y-6">
              <div className="space-y-2">
                <Input field={suggestionTitle} />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select
                  value={suggestionFor}
                  onChange={(e) => setSuggestionFor(e.target.value)}
                  className="w-full p-2.5 sm:p-3 border-2 border-[#4f46e5]/20 rounded-lg focus:ring-2 focus:ring-[#4f46e5]/50"
                  required
                >
                  <option value="">-- Select Option --</option>
                  <option value="Hostel">Hostel</option>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows="5"
                  className="w-full p-2.5 sm:p-3 border-2 border-[#4f46e5]/20 rounded-lg focus:ring-2 focus:ring-[#4f46e5]/50 resize-none"
                  onChange={handleDescChange}
                  value={desc}
                  required
                />
              ...
              <button
                type="submit"
                className="w-full bg-[#4f46e5] text-white py-2.5 sm:py-3 px-5 rounded-lg font-semibold hover:bg-[#4338ca] transition duration-200 text-sm sm:text-base"
                disabled={!title.trim() || !desc.trim() || count >= 2}
              >
                {count >= 2 ? "Daily Limit Reached" : "Submit Suggestion"}
              </button>
          ...
          {/* Suggestion History */}
          <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 w-full lg:w-1/2 min-h-[460px] sm:min-h-[500px] flex flex-col">
            <h2 className="text-[#4f46e5] font-semibold text-xl sm:text-2xl mb-4 text-center">
              Your Suggestions (Last 1 Month)
            </h2>

            {history.length === 0 ? (
              <p className="text-center text-gray-500 flex-grow flex items-center justify-center text-sm sm:text-base">
                No suggestions found.
              </p>
            ) : (
              <ul className="space-y-4 max-h-[400px] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
                {history.map((sugg, index) => (
                  <li
                    key={sugg._id || index}
                    className="border border-[#4f46e5]/20 rounded-lg p-3 sm:p-4 bg-[#f9f8ff]"
                  >
                    <h3 className="font-bold text-[#4f46e5] text-base sm:text-lg">
                      {sugg.title}
                    </h3>
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 mt-2">
                      For: <b>{sugg.suggestionFor}</b> | Status:{" "}
                      <span
                        className={
                          sugg.status === "pending"
                            ? "text-yellow-600"
